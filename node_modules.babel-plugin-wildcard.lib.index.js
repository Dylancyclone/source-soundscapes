  ...
  return {
    visitor: {
      ImportDeclaration: function ImportDeclaration(path, state) {
        ...
        var dir = _path2.default.join(_path2.default.dirname(name), src); // path of the target dir.


        for (var i = node.specifiers.length - 1; i >= 0; i--) {
          dec = node.specifiers[i];

          if (t.isImportNamespaceSpecifier(dec) && _fs2.default.existsSync(dir) && !_fs2.default.statSync(dir).isFile()) {
            addWildcard = true;
        ...
        if (addWildcard) {
          ...
          try {
            var r = _fs2.default.readdirSync(dir);

            for (var i = 0; i < r.length; i++) {
              // Check extension is of one of the aboves
              var _path$parse = _path2.default.parse(r[i]),
            ...
            console.warn("Wildcard for ".concat(name, " points at ").concat(src, " which is not a directory."));
            return;
          ...
          for (var i = 0; i < files.length; i++) {
            ...
            if (state.opts.nostrip !== true) {
              name = "./" + _path2.default.join(src, _path2.default.basename(file));
            } else {
              name = "./" + _path2.default.join(src, file);
            ...
            var importDeclaration = t.importDeclaration([isDir ? t.importNamespaceSpecifier(id) : t.importDefaultSpecifier(id)], t.stringLiteral(name)); // Assign it
